...
//Declare your list here and initialize PortB and PortF here
 ...
 PORTF_Init();
 unsigned char digitPattern[] = {0x40,0x3f,0x7f,0x5b,0x4f,0x6d}; //List of Hex Code for Digits
 int size = sizeof(digitPattern) / sizeof(digitPattern[0]);
...
// (GPIO_PORTF_DATA_RD & 0x10) == 0
 ...
 if ((GPIO_PORTF_DATA_RD & 0x10) == 0 ){
  for (i = 1; i < size; i++) {
  GPIO_PORTB_DATA_R = digitPattern[i]; // Write digit to PortB
...
counter = delay * 4000; // 1 second (1000 msec) needs 40000000 counter so4000000/1000 = 4000
for(i=0;i<counter;i++);
}
