...
int main(void)
{
	...
	SYSCTL->RCGCGPIO |= 0x--;   /* enable clock to GPIOF */
    GPIOF->DIR |= 0x--;         //set PF1, PF2 and PF3 as digital output pin
    GPIOF->DEN |= 0x--;         // CON PF1, PF2 and PF3 as digital GPIO pins
  	Delay(--); 
...
/* Send status string to Andriod app after turning on/off LEDs */
        ...
        if( c== --){ \ refer to the lab manual
			GPIOF->DATA |=(1<<x); /write down x
