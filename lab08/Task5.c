...
void Turn_Clockwise(void)
{
   SYSCTL->RCGCGPIO |=  0x1; /* enable clock to PORTF */
    GPIOA->DIR |=  (1 << 3) | (1 << 2);  /* Make PA3 and PA2 an output pin*/
     GPIOA->DEN |=  (1 << 3) | (1 << 2);/* Make PA3 and PA2 an digitally enabled*/
     GPIOA->DATA |=  (1 << 2);/* IN1 Should be HIGH */
   GPIOA->DATA &=  ~(1 << 3);/* IN2 Should be Low*/
}
...
void Turn_AntiClockwise(void)
{
    ...
    GPIOA->DIR |= (1 << 3) | (1 << 2);   /* Make PA3 and PA2 an output pin*/
     GPIOA->DEN |=  (1 << 3) | (1 << 2); /* Make PA3 and PA2 an digitally enabled*/
     GPIOA->DATA |= ~(1 << 2); /* IN1 Should be LOW */
   GPIOA->DATA &= (1 << 3);/* IN2 Should be HIGH*/}
